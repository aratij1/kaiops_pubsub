...
def _fallback_actionability(alert: Alert) -> tuple[bool, str]:
    ...
    # language (e.g. "memory usage is at 94%", "P99 latency exceeded
    # 4500ms", "deadlock exceptions", "signature verification error rate
    # spiked") that doesn't contain any of the specific tokens above but is
    # already a CRITICAL/HIGH severity signal from the monitoring source or
    # classify_severity(). Without this, correctly-classified critical/high
    # alerts were silently dropped as "noise" before ever reaching an
    # incident, with severity never consulted. Explicit noise keywords above
    # still take precedence so internal/test/retry chatter stays suppressed
    # even if it happens to carry a high severity label.
    if not actionable and alert.severity in (AlertSeverity.CRITICAL, AlertSeverity.HIGH):
        actionable = True
    return (
        actionable,
