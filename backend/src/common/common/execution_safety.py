...
def build_pre_execution_snapshot(action: RemediationAction) -> dict[str, Any]:
    # The snapshot is attached back to action.parameters after assessment.
    # Deep copies prevent a recursive parameters -> snapshot -> parameters
    # structure and ensure the evidence cannot change during execution.
    return {
...
def assess_execution_safety(
    ...
    normalized_allowlist = {str(item).strip().lower() for item in allowlisted_actions}

    action_type = snapshot["action_type"]
    if not action_type or action_type not in normalized_allowlist:
