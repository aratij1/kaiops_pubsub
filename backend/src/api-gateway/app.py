...
def _get_or_create_counter(name: str, documentation: str, labelnames: list[str] | tuple[str, ...] = ()) -> Counter:
    if name in REGISTRY._names_to_collectors:
        return REGISTRY._names_to_collectors[name]
    return Counter(name, documentation, labelnames)

def _get_or_create_gauge(name: str, documentation: str, labelnames: list[str] | tuple[str, ...] = ()) -> Gauge:
    if name in REGISTRY._names_to_collectors:
        return REGISTRY._names_to_collectors[name]
    return Gauge(name, documentation, labelnames)

GATEWAY_REQUESTS = _get_or_create_counter(
    "kaiops_gateway_requests_total",
    "API gateway requests by path and safety decision",
    ["path", "decision", "status"],
)
GATEWAY_SAFETY_BLOCKS = _get_or_create_counter(
    "kaiops_gateway_safety_blocks_total",
    "API gateway blocked requests by category",
    ["category"],
)
ALERTS_TABLE_ROWS = _get_or_create_gauge(
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            response = await client.request(
                method,
                target_url,
                # `payload or None` previously turned a legitimately empty
                # body (`{}` or `[]`) into no body at all, since both are
                # falsy in Python. That silently dropped the request body
                # for callers who intentionally POST `{}` (downstream
                # services fill in defaults for missing fields), producing
                # a spurious 422 "Field required" from the target service.
                # Only a payload that is actually absent (None) should omit
                # the JSON body.
