...
from common.telemetry import DASHBOARD_GENERATION_DURATION, ONBOARDING_FAILED, ONBOARDING_SUCCESS
    ...
    try:
        ...
        async with session_factory() as session:
            ...
            await repo.update_application_status(
                application.id,
                status="failed",
                payload={"dashboard": {"error": str(error), "failed": True}},
            )
            await repo.save_monitoring_audit(
                MonitoringAuditEvent(
                    application_id=application.id,
                    tenant_id=application.tenant_id,
                    event_type=f"{APPLICATION_DASHBOARD_CREATED}.failed",
                    actor="system",
...
async def startup(app: FastAPI) -> None:
    async def handle(payload: dict) -> None:
        ...
        try:
            ...
            ONBOARDING_FAILED.labels(settings.service_name, "dashboard").inc()
            logger.error(
                "dashboard generation stage failed",
