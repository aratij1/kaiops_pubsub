...
def test_workflow_engine_accepts_azure_service_bus_default_provider(tmp_path) -> None:
    config_path = tmp_path / "orchestration.json"
    config_path.write_text(
        json.dumps(
            {
                "message_bus": {
                    ...
                    "default_provider": "azure-service-bus",
                    "stream_threshold": 50,
                },
                "workflow_definitions": {
                    "critical-auto-remediation": {
    ...
    assert selection_low_stream.message_bus_provider == "azure-service-bus"
    assert selection_high_stream.message_bus_provider == "azure-service-bus"
