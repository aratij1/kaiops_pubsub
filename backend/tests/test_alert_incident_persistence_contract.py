...
@pytest.mark.asyncio
async def test_unified_inbox_excludes_noise_and_duplicate_alerts(sqlite_session_factory) -> None:
    """PASSED/actionable alerts must reach the Unified Inbox; alerts Alert
    Intelligence has already classified as noise (FAILED/WARNING, i.e.
    non-actionable) or as a duplicate merged into a canonical incident must
    not — even though neither leaves an IncidentOccurrenceRecord keyed to
    its own alert id (see backend/src/alert-intelligence/app.py
    _noise_classification and _merge_duplicate_into_canonical)."""
    now = datetime.now(UTC)
    passed_id = uuid4()
    failed_id = uuid4()
    warning_id = uuid4()
    ...
    async with sqlite_session_factory() as session:
        session.add(AlertRecord(
            id=passed_id, tenant_id="tenant-classified", source="prometheus",
            name="Actionable alert", service="checkout", environment="prod",
            severity="critical", fingerprint="passed",
            payload={"project_id": "commerce"}, created_at=now,
        ))
        session.add(AlertRecord(
            id=failed_id, tenant_id="tenant-classified", source="prometheus",
            name="Non-actionable alert", service="checkout", environment="prod",
            severity="warning", fingerprint="failed",
            payload={
                "project_id": "commerce",
                "metadata": {"noise": {"classified": True, "reason": "not actionable"}},
            },
            created_at=now,
        ))
        session.add(AlertRecord(
            id=warning_id, tenant_id="tenant-classified", source="prometheus",
            name="Routine warning alert", service="checkout", environment="prod",
            severity="warning", fingerprint="warning",
            payload={
                "project_id": "commerce",
                "metadata": {"deduplication": {"disposition": "noise"}},
            },
            created_at=now,
        ))
        session.add(AlertRecord(
            id=duplicate_id, tenant_id="tenant-classified", source="prometheus",
            name="Duplicate alert", service="checkout", environment="prod",
            severity="critical", fingerprint="duplicate",
            payload={
                "project_id": "commerce",
                "metadata": {"deduplication": {"disposition": "duplicate"}},
            },
            created_at=now,
        ))
        await session.commit()
    ...
    async with sqlite_session_factory() as session:
        page = await IncidentRepository(session).list_unified_inbox(
            tenant_id="tenant-classified", limit=10,
        )

    alert_ids_in_feed = {item["row"]["alert_id"] for item in page["rows"] if item["record_type"] == "alert"}
    assert alert_ids_in_feed == {str(passed_id)}
    assert str(failed_id) not in alert_ids_in_feed
    assert str(warning_id) not in alert_ids_in_feed
    assert str(duplicate_id) not in alert_ids_in_feed
