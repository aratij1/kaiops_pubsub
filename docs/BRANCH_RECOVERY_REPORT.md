...
Platform controls, deployment validation, and the complete CI gate remain pending. The recovery branch is not eligible for promotion until every mandatory backend, frontend, Playwright, Compose, Kubernetes, security, ancestry, and architecture gate passes. Promotion must occur through review of `recovery/kaims-consolidated-main`; `main` must not be force-pushed or replaced.
