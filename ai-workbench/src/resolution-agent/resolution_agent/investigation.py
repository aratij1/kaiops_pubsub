    ...
    def _revise_hypotheses(
        ...
        *,
        ...
        if not hypotheses:
            ...
            if diagnostic:
                ...
                hypotheses = [{
                    ...
                    # Evidence-matching below tokenizes "claim" to find corroborating
                    # rows. The fixed "Observed signal requiring causal
                    # confirmation:" prefix is boilerplate that never appears in
                    # real evidence text -- it only dilutes the token-overlap
                    # ratio against the one informative fragment (the snippet
                    # itself), which made otherwise-clearly-relevant evidence
                    # fail the >=2-token-or-35%-overlap test purely because of
                    # this template's own wording. match_text carries just the
                    # diagnostic snippet so matching is judged against the
                    # actual signal, not the label wrapped around it.
                    "match_text": snippet,
                    "status": "viable",
                    "confidence": 0.3,
        ...
        for hypothesis in hypotheses:
            claim_tokens = self._tokens(hypothesis.get("match_text") or hypothesis.get("claim"))
            support: list[str] = []
            sources: set[str] = set()
