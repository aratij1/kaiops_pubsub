...
test("resource explorer ignores an older cloud response after the project scope changes", async ({ page }) => {
  let releaseDemoProject;
  const demoProjectResponse = new Promise((resolve) => { releaseDemoProject = resolve; });
  await page.route("**/api-gateway/**", async (route) => {
    const request = route.request();
    ...
    if (path === "/cloud-ops/resources") {
      ...
      if (projectId === "demo-project") await demoProjectResponse;
      const displayName = projectId === "project-b" ? "project-b-api" : "stale-demo-api";
      return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ data: { rows: [{
        id: `${projectId}-resource`, project_id: projectId, connection_id: "connection-1", provider: "simulator",
        provider_resource_id: `simulator://${displayName}`, resource_type: "service", display_name: displayName,
        environment: "prod", service_id: displayName, status: "healthy",
      }], count: 1 } }) });
    }
    if (path.startsWith("/cloud-ops/services/") && path.endsWith("/topology")) {
      return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ data: { nodes: [], edges: [] } }) });
  ...
  await page.getByLabel("Password").fill("Admin@123456");
  await page.getByRole("button", { name: "Sign In" }).click();
  await page.getByLabel("Project ID").fill("project-b");
  await page.getByRole("button", { name: "Apply scope" }).click();
  await expect(page.getByText("project-b-api", { exact: true }).first()).toBeVisible();
  releaseDemoProject();
  await page.waitForTimeout(100);
  await expect(page.getByText("project-b-api", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("stale-demo-api", { exact: true })).toHaveCount(0);
});
