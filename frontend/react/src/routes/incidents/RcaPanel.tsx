...
export default function RcaPanel({
  ...
  // objects that happen to share a similar name: "investigation_report" is
  // the LLM RCA pipeline's evidence-coverage summary (schema
  // kaims.resolution-investigation.v1 -- coverage/missing_sources/etc, no
  // "conclusion", "status", or "conclusive" fields at all) while
  // "iterative_investigation" is the bounded, evidence-grounded
  // investigation graph's report and is the ONLY one of the two that ever
  // carries conclusion.confidence/status/conclusive. Because
  // investigation_report is always present once the resolution graph has
  // run, the previous `investigation_report || iterative_investigation`
  // order always picked the coverage-summary object, so every
  // ?.conclusion?.confidence / ?.conclusive / ?.status lookup silently
  // evaluated against undefined and rendered as 0% RCA confidence /
  // "review required" even when the iterative investigation had computed a
  // real, differentiated confidence score. iterative_investigation must be
  // preferred; investigation_report is kept only as a fallback for any
  // older recommendation that predates the iterative investigation report.
  const investigationReport = recommendationMetadata?.iterative_investigation
    || recommendationMetadata?.investigation_report
    || {};
