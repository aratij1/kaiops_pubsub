...
export default function RcaPanel({
  ...
  const investigationChecks = [
    { id: "evidence", label: "Linked observations", detail: evidenceRows.length ? `${evidenceRows.length} evidence record(s) linked.` : "No direct observations are linked.", passed: evidenceRows.length > 0, action: "collect metrics, logs, traces, or change evidence" },
    ...
    <section className="combined-analysis-page context-workspace">
      <header className="context-workspace-hero">
        ...
        <div className="context-workspace-summary" aria-label="Analysis summary">
          ...
          <dl>
            <div><dt>Confidence</dt><dd>{formatQualityPercent(confidence)}</dd></div>
            <div><dt>Live evidence</dt><dd>{freshEvidenceCount}</dd></div>
      ...
      {rcaDetailView === "evidence" ? <>
        <section className="ai-trust-panel evidence-review" aria-labelledby="ai-trust-title">
          ...
          <details className="evidence-ledger evidence-ledger-modern" open><summary><div><strong>Evidence ledger</strong><span>{evidenceRows.length} linked records with source, observation time, and citation</span></div><span>Show records</span></summary><div className="table-wrap ai-evidence-table contained-table"><table><thead><tr><th>Source</th><th>Observed</th><th>Freshness</th><th>Citation</th><th>Evidence type</th></tr></thead><tbody>{evidenceRows.length ? evidenceRows.map((row: any) => <tr key={row.id}><td><span className="source-badge">{row.source}</span></td><td>{row.timestamp ? formatUtcTimestamp(row.timestamp) : "Timestamp unavailable"}<small className="table-secondary">{row.age}</small></td><td><span className={`evidence-freshness ${row.cached ? "is-cached" : "is-fresh"}`}>{row.cached ? "Cached" : "Live"}</span><small className="table-secondary">{row.freshness}</small></td><td className="evidence-citation"><code title={row.citation || "Not supplied"}>{row.citation || "Not supplied"}</code></td><td>{row.cached ? "Historical context" : "Direct observation"}</td></tr>) : <tr><td colSpan={5}>No linked evidence records. Treat this recommendation as ungrounded and require human review.</td></tr>}</tbody></table></div></details>
