...
export default function RcaPanel({
  ...
  const diagnosticNextStep = missingEvidence.length
    ? `Collect ${missingEvidence.map((gap: any) => String(gap?.category || gap)).join(", ")} evidence and rerun analysis.`
    : "Collect another independent signal or causal observation and rerun analysis.";
  ...
  return (
    <section className="combined-analysis-page context-workspace">
      ...
      <section className="analysis-workbench" aria-labelledby="analysis-workbench-title">
        ...
        <section className="reasoning-trace" aria-labelledby="reasoning-trace-title"><header><span className="discovery-eyebrow">Explainability trace</span><h4 id="reasoning-trace-title">How the recommendation was formed</h4></header><ol><li><span>1</span><div><strong>Collect observations</strong><p>{evidenceRows.length ? `${evidenceRows.length} context records were collected; ${freshEvidenceCount} are live and ${supportingEvidenceRows.length} were accepted as RCA support.` : "No linked evidence records are available."}</p></div></li><li><span>2</span><div><strong>Assess causality</strong><p>{selectedRcaDecision?.rootCause || "No probable cause was produced."}</p></div></li><li><span>3</span><div><strong>{selectedAiTrust?.rcaReady === true ? "Review the governed response" : "Collect the next diagnostic evidence"}</strong><p>{selectedAiTrust?.rcaReady === true ? (selectedRcaDecision?.action || "Review the governed response.") : diagnosticNextStep}</p></div></li></ol></section>
