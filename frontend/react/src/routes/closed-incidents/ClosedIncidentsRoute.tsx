...
export default function ClosedIncidentsRoute() {
  ...
  return <section className="grid single-col"><article className="panel">
    ...
    <div className="table-wrap"><table><thead><tr><th>Incident</th><th>Service</th><th>Severity</th><th>Status</th><th>Closed At</th></tr></thead><tbody>{closed.rows.map((row, index) => <tr key={row.incident_id || index}><td>{row.incident_id || "-"}</td><td>{row.service || "-"}</td><td>{row.severity || "-"}</td><td><span className={`pill status-${String(row.status || "closed").toLowerCase()}`}>{row.status || "closed"}</span></td><td>{formatTime(row.closed_at || row.updated_at)}</td></tr>)}{!closed.rows.length && !closed.loading ? <tr><td colSpan={5}>No closed incidents available.</td></tr> : null}</tbody></table></div>
  </article></section>;
}
