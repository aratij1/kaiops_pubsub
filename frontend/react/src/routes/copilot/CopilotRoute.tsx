...
import "./CopilotCapabilities.css";
...
export default function CopilotRoute() {
  ...
  const capabilities = navigationForRole(copilot.role).filter((item) => item.id !== "copilot");
  ...
  return <section className="copilot-workspace">
    ...
    </main><aside className="copilot-side"><article className="copilot-next"><span className="discovery-eyebrow">Recommended next</span><h3>{recommended[0]}</h3><p>{recommended[1]}</p><button type="button" className="button-primary" onClick={recommended[2]}>Continue workflow</button></article><article className="panel copilot-scope"><header><h3>Available context</h3><span className={copilot.platformReady ? "is-ready" : ""}>{copilot.platformReady ? "Live" : "Partial"}</span></header><dl><div><dt>Applications</dt><dd>{copilot.projectCount}</dd></div><div><dt>Alert documents</dt><dd>{copilot.alertDocumentCount}</dd></div>{copilot.isAdministrator ? <div><dt>Users</dt><dd>{copilot.userCount}</dd></div> : null}</dl><p>Copilot searches and links only records and functionality permitted for your signed-in role.</p></article><nav className="panel copilot-shortcuts"><h3>All available functionality</h3><p className="copilot-shortcuts-intro">Ask a question or open any role-authorized workspace.</p>{capabilities.map((item) => <button type="button" key={item.id} onClick={() => navigate(item.path)}><span><strong>{item.label}</strong><small>{item.description}</small></span><b aria-hidden="true">→</b></button>)}</nav></aside></div>
  </section>;
}
